...
class AuthApiManager {
    ...
    func registerUser(newUser: UserModel,
                      completion: @escaping (Bool, String?) -> (Void)) {
        // validate newUser's data for uniqueness
        validateUserData(newUser: newUser) {
            unique, errorMessage in
            // if newUser's validated data is unique
            if let errorMessage = errorMessage {
                completion(false, errorMessage)
            } else if unique {
                // create FirebaseAuth for user
                self.createFirebaseAuthUser(newUser: newUser) {
                    userID, _ in
                    if let userID = userID {
                        // create document for user
                        self.createUserDocument(newUser: newUser, newUserDocumentID: userID) {
                            succes, errorMessage in
    ...
    private func validateUserData(newUser: UserModel,
                                  completion: @escaping (Bool, String?) -> (Void)) {
        // check if 'personalID' of newUser is already in use
        isUniquePersonalID(personalID: newUser.personalID,
                           completion: { unique, errorMessage in
            let errorMessage = unique ? "Personal ID already in use." : errorMessage
    ...
    private func createFirebaseAuthUser(newUser: UserModel, completion: @escaping ((String?, Error?) -> (Void))) {
        ...
        FirestoreManager.auth.createUser(withEmail: newUser.email, password: newUser.password, completion: { authResult, error in
        
            completion(authResult?.user.uid, error)
        })
    }
    
    private func createUserDocument(newUser: UserModel, newUserDocumentID: String,
                                    completion: @escaping (Bool, String?) -> (Void))  {
    ...
    private func isUniquePersonalID(personalID: String,
                                    completion: @escaping (Bool, String?) -> (Void)) {
        FirestoreManager.db.collection(usersCollection).whereField("personalID", isEqualTo: personalID)
